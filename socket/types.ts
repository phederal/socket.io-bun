...
export type AuthUserNotNull = Exclude<AuthUser, null>;
export type AuthSessionNotNull = Exclude<AuthSession, null>;
...
export const RESERVED_EVENTS: ReadonlySet<string | Symbol> = new Set<ClientReservedEvents | keyof NamespaceReservedEventsMap<never, never, never, never> | keyof SocketReservedEventsMap | keyof EventEmitterReservedEventsMap>(<const>[
	'connect',
	'connect_error',
	'disconnect',
	'disconnecting',
 ...
 */
export type EventNamesWithAck<Map extends EventsMap, K extends EventNames<Map> = EventNames<Map>> = IfAny<
	Last<Parameters<Map[K]>> | Map[K],
	K,
	K extends (Last<Parameters<Map[K]>> extends (...args: any[]) => any ? (FirstNonErrorArg<Last<Parameters<Map[K]>>> extends void ? never : K) : never) ? K : never
>;
/**
 * Returns a union type containing all the keys of an event map that have an acknowledgement callback.
 *
 * That also have *some* data coming in.
 */
export type EventNamesWithoutAck<Map extends EventsMap, K extends EventNames<Map> = EventNames<Map>> = IfAny<
	Last<Parameters<Map[K]>> | Map[K],
	K,
	K extends (Parameters<Map[K]> extends never[] ? K : never) ? K : K extends (Last<Parameters<Map[K]>> extends (...args: any[]) => any ? never : K) ? K : never
>;
...
export type EventNamesWithError<Map extends EventsMap, K extends EventNamesWithAck<Map> = EventNamesWithAck<Map>> = IfAny<
	Last<Parameters<Map[K]>> | Map[K],
	K,
	K extends (LooseParameters<Last<Parameters<Map[K]>>>[0] extends Error ? K : never) ? K : never
>;
 ...
 */
export type ReservedOrUserListener<ReservedEvents extends EventsMap, UserEvents extends EventsMap, Ev extends ReservedOrUserEventNames<ReservedEvents, UserEvents>> = FallbackToUntypedListener<
 ...
 * @link https://github.com/sindresorhus/type-fest
 */
export type Last<ValueType extends readonly unknown[]> = ValueType extends readonly [infer ElementType]
	? ElementType
...
type ExpectMultipleResponses<T extends any[]> = {
	[K in keyof T]: T[K] extends (...args: infer Params) => infer Result
		? Params extends [Error]
			...
			? (err: Error, ...args: InferFirstAndPreserveLabel<MultiplyArray<Rest>>) => Result
			: Params extends []
			? () => Result
