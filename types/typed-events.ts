...
/**
 * An empty events map, used to represent an empty event emitter.
 */
...
export type EventNamesWithAck<Map extends EventsMap, K extends EventNames<Map> = EventNames<Map>> = IfAny<
	...
	K,
	K extends (Last<Parameters<Map[K]>> extends (...args: any[]) => any ? (FirstNonErrorArg<Last<Parameters<Map[K]>>> extends void ? never : K) : never)
		? K
		: never
>;
/**
...
export type EventNamesWithoutAck<Map extends EventsMap, K extends EventNames<Map> = EventNames<Map>> = IfAny<
	...
	K extends (Parameters<Map[K]> extends never[] ? K : never)
		? K
		: K extends (Last<Parameters<Map[K]>> extends (...args: any[]) => any ? never : K)
		? K
		: never
>;
...
> = FallbackToUntypedListener<Ev extends EventNames<ReservedEvents> ? ReservedEvents[Ev] : Ev extends EventNames<UserEvents> ? UserEvents[Ev] : never>;
 ...
 * `listen`.
 */
export abstract class StrictEventEmitter<ListenEvents extends EventsMap, EmitEvents extends EventsMap, ReservedEvents extends EventsMap = {}>
	...
	implements TypedEventBroadcaster<EmitEvents>
{
	 ...
	 */
	override on<Ev extends ReservedOrUserEventNames<ReservedEvents, ListenEvents>>(
		ev: Ev,
	 ...
	 * @param listener Callback function
	 */
	override once<Ev extends ReservedOrUserEventNames<ReservedEvents, ListenEvents>>(
		ev: Ev,
		listener: ReservedOrUserListener<ReservedEvents, ListenEvents, Ev>,
	): this {
	 ...
	 */
	override listeners<Ev extends ReservedOrUserEventNames<ReservedEvents, ListenEvents>>(
		event: Ev,
